...
server_aliases = [ "#{app['id']}.#{node[:domain]}", node.fqdn ].concat((app['server_aliases'] || {})[node.chef_environment] || [])
...
web_app app['id'] do
  ...
  cookbook "#{app['id']}"
  server_name (app['domain_name'] || {})[node.chef_environment] || "#{app['id']}.#{node[:domain]}"
